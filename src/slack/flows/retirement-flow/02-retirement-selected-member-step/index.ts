...
export const retirementSelectedMemberStep = async (teamId: string | undefined, channelId: string | undefined, userId: string, selectedMemberSlackId: string, { client, env }: SlackHandlerOptions) => {
  try {
    await apiClient.POST('/members/_rpc/create-bulk-member-status-log', {
      body: { __updaterSlackId: userId, body: { status: 'LEAVE' }, memberIds: [selectedMemberSlackId] },
    });
  } catch (error) {
