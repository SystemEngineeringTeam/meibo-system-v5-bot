import type { AnyMessageBlock, SlackAppContext } from 'slack-cloudflare-workers';
import type { HonoSlackAppEnv } from '@/types/hono';
import type { PayeeData } from '@/types/kv';
import { getNotifyChannelId } from '@/slack/lib/get-notify-channel-id';
import { kv } from '@/utils/kv';

type MemberType = '新入部員' | '継続部員';

export interface ConfirmRegistrationApprovalStepResult {
  success: boolean;
  // 送信済み/申請内容なし/システムエラー
  reason?: 'no_request_data' | 'error';
}

export const baseConfirmRegistrationApprovalStep = (approveActionId: string, rejectActionId: string, memberType: MemberType) => async (payerSlackUserId: string, payeeName: string, teamId: string | undefined, context: SlackAppContext, env: HonoSlackAppEnv): Promise<ConfirmRegistrationApprovalStepResult> => {
  const notifyChannelId = await getNotifyChannelId(teamId, env);

  const payeeData = await kv.get<PayeeData>(env.PAYEE_KV, payeeName);

  try {
    await context.client.chat.postMessage({
      channel: notifyChannelId,
      text: generateText(memberType, payerSlackUserId, payeeData?.slackUserId),
      blocks: generateBlocks(approveActionId, rejectActionId, memberType, payerSlackUserId, payeeData?.slackUserId),
      mrkdwn: true,
      metadata: {
        event_type: 'approval_request',
        event_payload: {
          payerSlackUserId,
        },
      },
    });
  } catch (error) {
    console.error('Failed to send confirmation message:', error);
    return { success: false, reason: 'error' };
  }

  return { success: true };
};

export const baseClickedApproveOrRejectButton = () => async (approve: boolean, channelId: string, timestamp: string, blocks: AnyMessageBlock[] | undefined, context: SlackAppContext) => {
  const activeActionId = approve ? 'member_reject' : 'member_approve';

  await context.client.chat.update({
    channel: channelId,
    ts: timestamp,
    text: approve ? '承認しました' : '拒否しました',
    blocks: blocks && [blocks[0], {
      type: 'actions',
      elements: blocks[1].type !== 'actions'
        ? []
        : blocks[1].elements.map((element) => ({
            ...element,
            style: element.action_id === activeActionId ? (approve ? 'danger' : 'primary') : undefined,
          })),
    }],
  });
};

function generateText(memberType: MemberType, payerSlackUserId: string, payeeSlackUserId?: string): string {
  // ユーザIDがあればメンション、なければ @channel
  const mention = payeeSlackUserId ? `<@${payeeSlackUserId}>` : '<!channel>';

  // TODO: ユーザの登録情報を付加する
  return `${mention} *【${memberType}】承認依頼が届いています* <@${payerSlackUserId}> `;
};

export function generateBlocks(approveActionId: string, rejectActionId: string, memberType: MemberType, payerSlackUserId: string, payeeSlackUserId?: string): AnyMessageBlock[] {
  const mention = payeeSlackUserId ? `<@${payeeSlackUserId}>` : '<!channel>';

  return [
    {
      type: 'section',
      text: {
        type: 'mrkdwn',
        text: `${mention} *【${memberType}】承認依頼が届いています*
申請者: <@${payerSlackUserId}>`,
      },
    },
    {
      type: 'actions',
      elements: [
        {
          type: 'button',
          text: {
            type: 'plain_text',
            text: '承認',
            emoji: true,
          },
          style: 'primary',
          value: payeeSlackUserId,
          action_id: approveActionId,
        },
        {
          type: 'button',
          text: {
            type: 'plain_text',
            text: '拒否',
            emoji: true,
          },
          style: 'danger',
          value: payeeSlackUserId,
          action_id: rejectActionId,
        },
      ],
    },
  ];
};
